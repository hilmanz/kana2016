...
class oauth_twitter_widget extends WP_Widget {
	...
	function oauth_twitter_widget() {
		...
		parent::WP_Widget( /* Base ID */'oauth_twitter_widget', /* Name */'Twitter Widget', array( 'description' => 'Displays your latest twitter.com updates' ) );
	}
	...
	?>
		<p>
			...
			<label for="<?php echo $this->get_field_id('oAuthAccessToken'); ?>" style="line-height:35px;display:block;">OAuth Access Token: <input style="width:90%" type="text" id="<?php echo $this->get_field_id('oAuthAccessToken'); ?>" size="38" name="<?php echo $this->get_field_name('oAuthAccessToken'); ?>" value="<?php echo $oAuthAccessToken; ?>" /></label>
			<label for="<?php echo $this->get_field_id('oAuthAccessTokenSecret'); ?>" style="line-height:35px;display:block;">Access Token Secret: <input type="text" style="width:90%" id="<?php echo $this->get_field_id('oAuthAccessTokenSecret'); ?>" size="38" name="<?php echo $this->get_field_name('oAuthAccessTokenSecret'); ?>" value="<?php echo $oAuthAccessTokenSecret; ?>" /></label>
			<label for="<?php echo $this->get_field_id('consumerKey'); ?>" style="line-height:35px;display:block;">Consumer Key: <input style="width:90%" type="text" id="<?php echo $this->get_field_id('consumerKey'); ?>" size="38" name="<?php echo $this->get_field_name('consumerKey'); ?>" value="<?php echo $consumerKey; ?>" /></label>
			<label for="<?php echo $this->get_field_id('consumerSecret'); ?>" style="line-height:35px;display:block;">Consumer Secret: <input style="width:90%" type="text" id="<?php echo $this->get_field_id('consumerSecret'); ?>" size="38" name="<?php echo $this->get_field_name('consumerSecret'); ?>" value="<?php echo $consumerSecret; ?>" /></label>
			<p>&nbsp;</p>
            ...
            <a href="https://dev.twitter.com/apps" target="_blank">Create Your APP Here to get Token Code</a>
		</p>
<?php
	}
	...
	function widget($args, $instance) {
		...
		if( $haveTwitterData ){
			...
			$pluginURL = get_bloginfo('template_directory')."/functions/twitter/";
			...
			foreach( $tweets as $tweet ){
				...
				echo "<div class='rowtwit'>";
					if( $showTwitterIconTF ){
						echo "<div class='thumbtwitter'>";
				...
				if( $showTweetTimeTF ){
					echo "<div class=\"oauth-twitter-tweet-time\" id=\"oauth-twitter-tweet-time-" . $i . "\">" . $this->twitter_time_ltw( $tweet->created_at ) . "</div></div>";
				}
				$i++;
			}
		} else{
...
if( !function_exists('oauth_twitter_widget_css')){
	function oauth_twitter_widget_css( ){
		echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"" . get_bloginfo('template_directory') ."/functions/twitter/twitter_widget.css\" />" . "\n";
	}
